...
namespace LibraryManager.Application.Commands.Book
{
    public class UpdateBookCommand : IRequest<Result>
    {
        ...
        public UpdateBookCommand(int id, int bookCode, string title, string publisher, int edition, int publishYear, decimal price, List<int>? authorIds, List<int>? subjectIds)
        {
            Id = id;
            BookCode = bookCode;
            Title = title;
